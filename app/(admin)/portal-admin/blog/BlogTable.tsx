...
import { Pill, StatusPill } from "../_components/admin-ui";
...
export function BlogTable({ rows }: { rows: CmsBlogPost[] }) {
  ...
  const filtered = useMemo(() => {
    ...
    if (q.trim()) {
      ...
      r = r.filter((x) => x.title.toLowerCase().includes(lc) || (x.author ?? "").toLowerCase().includes(lc));
    }
    return r;
  }, [rows, tab, q]);
  ...
  const handleDelete = (id: string, title: string) => {
    ...
    setPendingId(id);
    start(async () => {
      try { await deleteBlogPost(id); router.refresh(); } finally { setPendingId(null); }
    });
  };
    ...
    <div className="adm-panel">
      <div className="adm-tbl-toolbar">
        <div className="adm-tbl-tabs">
          {([["all", "All"], ["published", "Published"], ["scheduled", "Scheduled"], ["draft", "Draft"]] as const).map(([id, label]) => (
        ...
        <Link href="/portal-admin/blog/new" className="adm-btn-sm primary">
      ...
      <div className="adm-panel-body flush">
        <table className="adm-tbl">
          <thead><tr><th>Title</th><th>Author</th><th>Category</th><th>Published</th><th>Views</th><th>Status</th><th /></tr></thead>
          <tbody>
            {filtered.length === 0 ? (
              <tr><td colSpan={7} style={{ textAlign: "center", padding: 32, color: "var(--text-3)" }}>No articles match.</td></tr>
            ) : (
              filtered.map((p) => (
                <tr key={p.id} style={pendingId === p.id ? { opacity: 0.5 } : undefined}>
                  ...
                  <td>{p.author ?? <span style={{ color: "var(--text-3)" }}>—</span>}</td>
                  <td><Pill tone="gray">{p.category}</Pill></td>
                  <td>{fmt(p.publishedAt)}</td>
                  <td>{p.views}</td>
                  <td><StatusPill status={p.status} /></td>
                  <td>
                    <div className="adm-row-actions">
                      <Link href={`/portal-admin/blog/${p.id}/edit`} className="adm-ra-btn" title="Edit">
                      ...
                      <a href={`/blog/${p.slug}`} target="_blank" rel="noopener noreferrer" className="adm-ra-btn go" title="View on site">
