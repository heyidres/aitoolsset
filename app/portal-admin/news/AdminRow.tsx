...
export function AdminRow({ post }: { post: RowPost }) {
  ...
  const change = (status: RowPost["status"]) => {
    start(async () => {
      setErr(null);
      const res = await fetch(`/api/portal-admin/news/${post.id}`, {
        method: "PATCH",
  ...
  return (
    <div className="flex items-center gap-4 px-5 py-4 flex-wrap" style={{ borderBottom: "1px solid var(--border)" }}>
      ...
      <div className="flex-1 min-w-0">
        ...
        <Link href={`/portal-admin/news/${post.id}/edit`} className="font-display text-[14.5px] font-extrabold leading-[1.35] mt-1 block hover:text-blue">
          {post.headline}
        </Link>
      ...
      <div className="flex items-center gap-2 flex-shrink-0">
        ...
        <Link
          href={`/portal-admin/news/${post.id}/edit`}
