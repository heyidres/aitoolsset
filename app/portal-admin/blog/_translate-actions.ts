...
function isProviderUnavailable(err: unknown): boolean {
  ...
  return (
    ...
    // limit (e.g. Groq's free-tier TPM cap on a long article body) — try
    // the next provider in the chain rather than giving up entirely.
    msg.includes("413") ||
...
export async function translateBlogPostUnauthenticated(
  ...
  try {
    ...
    revalidatePath(`/portal-admin/blog/${postId}/edit`);
...
/**
 * Translate ONE post into EVERY non-default locale, one click instead of
 ...
 */
export async function autoTranslateBlogPostAllLocales(
  postId: string,
  ...
  try {
    ...
    for (let i = 0; i < targets.length; i++) {
      ...
      const result = await translateBlogPostUnauthenticated(postId, targets[i], user.id);
      results.push(
        result.ok
