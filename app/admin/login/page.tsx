 ...
 * /admin/login. Magic-link + optional Google, gated by Turnstile and a
...
export default async function AdminLoginPage({
  ...
  if (session?.user) redirect("/admin/2fa");

  const sp = await searchParams;
  const callbackUrl = sp.callbackUrl && sp.callbackUrl.startsWith("/admin") ? sp.callbackUrl : "/admin";
