...
/**
 * Shared form for /admin/news/new (manual create) and
 * /admin/news/[id]/edit (review AI-generated drafts).
 *
 ...
 *
 * Save flow:
 *   - create mode → POST /api/admin/news
 *   - edit mode   → PATCH /api/admin/news/[id]
 *
...
export function NewsForm({
  ...
  const submit = (e: React.FormEvent) => {
    ...
    start(async () => {
      ...
      if (mode === "create" && j.id) router.push(`/admin/news/${j.id}/edit`);
      else router.refresh();
    });
  };
    ...
    <form className="adm-input-wrap" onSubmit={submit}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", marginBottom: 20, flexWrap: "wrap", gap: 12 }}>
        ...
        <div style={{ display: "flex", gap: 8 }}>
          <Link href="/admin/news" className="adm-btn-sm ghost">← Back to queue</Link>
          {mode === "edit" && postSlug && (
            <a
              href={`/news/${postSlug}`}
              target="_blank"
